import {MODULES,PROJECTS,RECIPIENT} from './config.js';
...
async function submitForm(e){e.preventDefault();if(!$('#dataConsent').checked){markError($('#dataConsent').closest('.acceptance'),'Debes autorizar la revisión de la información.');return;}const btn=$('#submitBtn'),msg=$('#submitMessage');btn.disabled=true;btn.textContent='Preparando entrega...';const data=collectData();downloadWorkbook(data);
  try{const payload=new FormData();payload.append('data',JSON.stringify({...data,rawFiles:undefined}));const excel=workbookBlob(data);payload.append('excel',excel,`ESG_${data.projectCode}.xlsx`);data.rawFiles.forEach(f=>payload.append('attachments',f));const r=await fetch('/api/submit',{method:'POST',body:payload});const out=await r.json().catch(()=>({}));if(!r.ok)throw new Error(out.error||'No se pudo enviar automáticamente.');msg.className='notice';msg.innerHTML=`<strong>Proyecto enviado.</strong><br>Código: ${data.projectCode}. También descargamos una copia en Excel para tus archivos.`;btn.textContent='Enviado';}
  catch(err){msg.className='notice warning';msg.innerHTML=`<strong>Tu Excel ya fue descargado.</strong><br>El envío automático todavía no está configurado o los archivos exceden el límite. Adjunta el Excel descargado y los materiales originales a <a href="mailto:${RECIPIENT}?subject=${encodeURIComponent('[ESG EXPERIENCE] Proyecto '+data.projectCode+' — '+data.business.businessName)}">${RECIPIENT}</a>, o envíalos por WhatsApp como documento para conservar la calidad.<br><small>${err.message}</small>`;btn.disabled=false;btn.textContent='Intentar enviar otra vez';}
}
init();
